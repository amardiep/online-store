...
def signup(request):
    if request.method=="POST":
        ...
        if password != password1:
            messages.warning(request,"लवड्याच्या बाला एकदा दोन्ही पासवर्ड बघून घे")
            return render(request,'authentication/signup.html')
        try:
            if User.objects.get(username=email):
                messages.info(request,"Already registered this email")
                return render(request,'authentication/signup.html')
        ...
        email_message.send()
        messages.success(request,"Activate your account by clicking the link on gmail")
